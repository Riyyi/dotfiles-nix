...
I'm using ZFS on my NAS and those need to be formatted only once, therefor their
disko config is in a separate file, not executed in the main install. These
commands need to be run after installing the distro, but before reboot!

#+BEGIN_SRC sh
sudo nix --experimental-features "nix-command flakes" run github:nix-community/disko/latest -- --mode destroy,format,mount /mnt/etc/nixos/profiles/nixos-nas/disko-mount.nix
sudo zfs unmount -a
